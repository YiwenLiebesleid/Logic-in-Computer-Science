...
(* forall P : booltree -> Prop,
		...
		(forall (b : bool) (t1 : booltree), P t1 ->
				...
				P ( bt_branch b t1 t2)) -> forall (b : booltree), P b
*)
...
Definition def_plus_assoc (n : nat) : Prop := forall m p : nat,
	n + (m + p) = (n + m) + p.

Theorem plus_assoc'' : forall n,
	def_plus_assoc n.
Proof.
	apply nat_ind.
	- unfold def_plus_assoc. intros m p. simpl. reflexivity.
	- intros n. unfold def_plus_assoc. intros H. intros m p.
		simpl. rewrite H. reflexivity.
Qed.

Definition def_plus_comm (n : nat) : Prop := forall m : nat,
	n + m = m + n.

Theorem plus_comm''' : forall n,
	def_plus_comm n.
Proof.
	apply nat_ind.
	- unfold def_plus_comm. intros m. simpl. rewrite <- plus_n_O. reflexivity.
	- intros n. unfold def_plus_comm. intros H.
		intros m. simpl. rewrite H. rewrite <- plus_n_Sm. reflexivity.
Qed.
(* FILL IN HERE
