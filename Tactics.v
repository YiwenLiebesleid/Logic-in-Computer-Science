...
(* FILL IN HERE
		Apply is similar as rewrite -> H, but it will try to rewrite the entire subgoal, while rewrite will replace the matched part.
    [] *)
...
Theorem list_app_eq : forall (X : Type) (x y : X) (l : list X),
	x :: l = y :: l -> x = y.
Proof.
	intros X x y l H.
	injection H as H1.
...
(* For all n, suppose we have IH: forall m : nat, (n' =? m) = true -> n' = m. Then,
	when n = 0, for all m, we have the equation (0 =? m) = true is true, since when m = 0, it straightforwardly stands, while when m = Sm', 0 = Sm' is always false.
	when n = Sn', for all m, we have the equation (S n' =? m) = true is true, since when m = 0, Sn' = 0 is always false, while when m = Sm', we can use the euation and f_equal to prove it's true.
*)
...
Proof.
	...
	- simpl. intros m eq. destruct m as [| m'] eqn:E.
		...
		+ simpl in eq.
			...
			apply f_equal. apply IHn'. apply goal.
Qed.
(** [] *)
...
Proof.
	intros f b.
	destruct (f (f b)) eqn:Hffb.
	- destruct (f b) eqn:Hfb.
		+ apply Hffb.
		+ destruct b eqn:Hb.
			* apply Hfb.
			* rewrite Hffb in Hfb. discriminate Hfb.
	- destruct (f b) eqn:Hfb.
		+ destruct b eqn:Hb.
			* rewrite Hffb in Hfb. discriminate Hfb.
			* apply Hfb.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal) 
   ...
   Proof: *)
   (* for all n, when n = 0, for all m, we can prove this straightforwardly. when n = Sn', and m = 0, we can prove this straightforwardly, while if m = Sm', we can use hypothsis to prove this.
    [] *)

(** **** Exercise: 3 stars, standard, optional (eqb_trans)  *)
Theorem eqb_eq_rev : forall n m,
	n = m -> n =? m = true.
Proof.
	intros n.
	induction n as [| n' IHn'].
	- intros. rewrite <- H. reflexivity.
	- intros. destruct m as [| m'] eqn:Em.
		+ discriminate.
		+ apply IHn'. injection H as H1. apply H1.
Qed.
...
Proof.
	...
	- apply eqb_true in H. apply H.
	- apply eqb_true in H0. apply H0.
Qed.
(** [] *)
...
Theorem list_length_eq_zero : forall (X : Type) (l : list X),
	0 = length l -> l = [].
Proof.
	intros. induction l.
	- reflexivity.
...
Proof.
	intros X l1.
	induction l1.
	- simpl. intros. apply list_length_eq_zero in H.
		rewrite H. reflexivity.
	- intros. destruct l2 as [| l2'] eqn:E2.
		+ simpl in H. discriminate H.
		+ simpl in H. injection H as H1. apply IHl1 in H1. simpl.
...
Proof.
	...
	- intros. simpl in H. destruct (test h) eqn:Eh.
		+ injection H as H1 H2. symmetry in H1. rewrite H1.
			apply Eh.
		+ apply IH with lf. apply H.
Qed.
(** [] *)
...
Proof.
	...
	induction l.
	- reflexivity.
	- simpl. unfold existsb'. rewrite -> IHl.
