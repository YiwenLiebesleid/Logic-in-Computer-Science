...
The 1st [->>]: since X <= Y, we can rewrite Z := Y - X in Y = X + Z.
The 2nd [->>]: since ~(X <= Y), we can rewrite Y := X + Z in Y = X + Z.
*)
...
**)
(**
	(1)->>(2) obviously, move X in (1) to the other side of equation.
	(4)->>(5) since we have X!=0, we are guaranteed the substraction doesn't get zero-truncated. The 1's on the left and right of the equation (5) have been cancelled up, so we can rewrite (5) as Y = m - X.
	(8)->>(9) we have X=0, then Y = m-0 = m, straightforwardly.
**)
...
(**
	(1)->>(2) obviously.
	(3)->>(4) since we have X!=0, we are guaranteed the substraction doesn't get zero-truncated. The 1's cancelled up in (4) so we can rewrite (4) as X + Z = m + n.
	(7)->>(8) we have X=0, then X + Z = m + n => 0 + Z = m + n => Z = m + n, straightforwardly.
**)
...
Proof.
  ...
  - eapply hoare_consequence_pre.
		+ apply hoare_asgn.
		+ intros st [Hp Hb]. unfold assn_sub, update.
			simpl. rewrite <- Hp. apply parity_ge_2.
    	unfold bassn, beval, aeval in Hb. apply leb_complete in Hb. auto.
	- intros st. verify_assn.
  - intros st [Hp Hb]. rewrite <- Hp. symmetry. apply parity_lt_2.
    unfold bassn, beval, aeval in Hb.
		rewrite not_true_iff_false in Hb.
    apply leb_complete_conv in Hb. unfold not. intros.
		apply leb_correct in H. apply leb_correct_conv in Hb.
		simpl in  *. rewrite Hb in H. inversion H. 
Qed.
(** [] *)
...
(** **** Exercise: 3 stars, standard (factorial) 
  ...
  do   {{ Y * X! = m! /\ X <> 0 }} ->>
    ...
    Briefly justify each use of [->>].
The 1st [->>]: it holds obviously.
The 2nd [->>]: since we have X<>0, we are guaranteed the multiplication doesn't get zero-truncated. We have (Y*X)*(X-1)!= Y*X*(X-1)! =Y*X! Thus, we can rewrite it as Y * X! = m!.
The 3rd [->>]: if we have X=0, then Y*X!=m! => Y*1=m! => Y=m!.
*)
...
(**
	The 1st [->>]: 1=2*1-1 is always true, 2^0=1 is always true.
	The 2nd [->>]: if we have X <> m, we still haven't finished the computation. We can rewrite the following line as Y = 2 * Z - 1 /\ Z = 2 ^ X, since it's obvious that Y+2*Z=2*2*Z-1=2*Z+2*Z-1 => Y=2*Z-1, and it's also obvious that 2*Z=2^(X+1)=2^X*2 => Z=2^X. Thus, we can rewrite them according to this.
	The 3rd [->>]: if we have X=m, we have finished the series' computation. Just plug in the values: Y=2*Z-1 =2*2^X-1 =2*2^m-1 =2^(m+1)-1 => Y=2^(m+1)-1.
*)
...
Proof.
	...
	- unfold "->>", hoare_triple. intros.
		...
		+ simpl in *. rewrite t_update_eq in H0. lia.
...
Proof.
	...
	- eapply hoare_consequence_pre.
		...
		+ verify_assn.
	- unfold "->>", hoare_triple. intros.
		apply H with (st':= (X !-> aeval st a; st) ) in H0.
		+ apply H0.
		+ constructor. reflexivity.
Qed.
(** [] *)
