...
(* holds if [X] according to st is less or equal than [Y] according to st *)
Definition assn2 : Assertion :=
  fun st => st X = 3 \/ st X <= st Y.
(* holds if [X] according to st equal to 3, or [X] according to st is less or equal than [Y] according to st *)
Definition assn3 : Assertion :=
  fun st => st Z * st Z <= st X /\
            ~ (((S (st Z)) * (S (st Z))) <= st X).
(* holds if the square of [Z] according to st is less or equal to [X] according to st, and the square of [Z] according to st plus 1 is NOT less or equal than [X] according to st *)
Definition assn4 : Assertion :=
  fun st => st Z = max (st X) (st Y).
(* holds if [Z] according to st equal to the maximum value between [X] according to st and [Y] according to st *)
End ExAssertions.
(** [] *)
...
(** **** Exercise: 1 star, standard, optional (triples) 
    ...
    Paraphrase the following Hoare triples in English.
     ...
     1) {{True}} c {{X = 5}}
	start with any state, then execute c, finally go to X=5
     2) forall m, {{X = m}} c {{X = m + 5)}}
	start with X=m, then execute c, finally go to X=m+5
     3) {{X <= Y}} c {{Y <= X}}
	start with X<=Y, then execute c, finally go to Y<=X
     4) {{True}} c {{False}}
	start with any state, then execute c, finally go to no state
     5) forall m,
          {{X = m}}
          c
          {{Y = real_fact m}}
	start with X=m, then execute c, finally go to Y=m!
     6) forall m,
          {{X = m}}
          c
          {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}
	start with X=m, then execute c, finally go to an m in range [Z*Z, (Z+1)*(Z+1))
*)
...
Proof.
	...
	unfold not in *. apply H in H1. destruct H1.
Qed.
(** [] *)
...
(* A possibleconter example can be some expressions like a : X + 1. Because in the precondition, we have no mention of X, and the command c is related to X. This makes the postcondition has X = X + 1, which is obviously incorrect.
 *)
...
Proof.
	...
	- inversion H. rewrite t_update_shadow. destruct H0.
		subst. rewrite t_update_same. apply H0.
	- inversion H. rewrite t_update_shadow. destruct H0.
		subst. rewrite t_update_same. apply t_update_eq.
Qed.
(** [] *)
...
Proof.
	...
	inversion H. exists (st X). subst. split.
...
Proof.
	eapply hoare_consequence_pre.
	- apply hoare_asgn.
	- unfold "->>", assn_sub. simpl. intros. rewrite t_update_eq. lia.
Qed.
...
Proof.
  ...
  (* The annotation [%assertion] is needed here to help Coq parse correctly. *)
	- eapply hoare_consequence_pre. apply hoare_asgn. unfold "->>", assn_sub, t_update. simpl. auto.
	- eapply hoare_consequence_pre. apply hoare_asgn. unfold "->>", assn_sub, t_update. simpl. auto.
Qed.
(** [] *)
...
Proof.
	...
	- eapply hoare_consequence_pre.
		+ apply hoare_asgn.
		+ unfold "->>", assn_sub, update. simpl. intros. auto.
Qed.
(** [] *)
...
Proof.
  ...
  intros H.
	...
	specialize H with (st:= empty_st) (st':= (X !-> 3)).
	eapply hoare_consequence_pre.
	- eapply hoare_asgn.
	- unfold "->>", assn_sub. intros. destruct H; admit.
...
Proof.
	apply hoare_if; eapply hoare_consequence_pre;
	try apply hoare_asgn; unfold bassn, assn_sub, update, assert_implies.
	- simpl. intros. destruct H. apply le_plus_minus.
	apply leb_le in H0. auto.
	- simpl. intros. destruct H. reflexivity.
Qed.
(** [] *)
...
Proof.
	...
	inversion H1; subst.
	- apply (H st st').
		+ apply H8. + split. apply H2. apply H5.
	- apply (H0 st' st').
		+ apply E_Skip.
		+ split. * apply H2. * unfold not. intros.
			simpl in *. rewrite H7 in H3. inversion H3.
Qed.
...
(* Example hoare_if1_good :
	...
	{{fun st => st X = st Z}}.
Proof.
	eapply hoare_if1; intros st st' H H1.
	- inversion H; subst. simpl. rewrite t_update_eq. rewrite t_update_neq.
		+ destruct H1. apply H0. + destruct H1. unfold not in *. intros.
			inversion H2.
	- inversion H; subst. destruct H1. unfold bassn in H1. unfold not in *.
		simpl in *. destruct (negb (st' Y =? 0)) eqn:E.
		rewrite negb_true_iff in E. rewrite eqb_neq in E.
		+ unfold not in *. rewrite <- H0.
			assert (a: st' X = st' X + 0). { rewrite add_0_r. reflexivity. }
			rewrite a. assert (a1: 0 + st' Y = st' Y). { reflexivity. }
			rewrite <- plus_assoc. rewrite a1.  apply add_cancel_l.
			symmetry. contradiction.
		+ rewrite negb_false_iff in E. rewrite eqb_eq in E.
			rewrite E in H0. rewrite add_0_r in H0. apply H0.
Qed. *)
(* Do not modify the following line: *)
