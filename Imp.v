...
(* reduced by 23 *)
Theorem optimize_0plus_sound_auto: forall a,
  aeval (optimize_0plus a) = aeval a.
Proof.
	intros; induction a;
	simpl; auto.
...
Proof.
	intros. induction b; try (simpl; reflexivity);
...
(* reduced by 7 *)
Theorem optimize_0plus_b_sound_auto : forall b,
  beval (optimize_0plus_b b) = beval b.
Proof.
	induction b; simpl;
...
Fixpoint my_optimize (a: aexp) : aexp :=
  match a with
  | ANum n => ANum n
  | APlus (ANum 0) e2 => my_optimize e2  (* 1 *)
(*  | APlus e1 (ANum 0) => my_optimize e1  (* 2 *) *)
  | APlus  e1 e2 => APlus  (my_optimize e1) (my_optimize e2)
(*  | AMinus e1 (ANum 0) => my_optimize e1  (* 3 *) *)
  | AMinus (ANum 0) e2 => ANum 0  (* 4 *)
  | AMinus e1 e2 => AMinus (my_optimize e1) (my_optimize e2)
  | AMult  (ANum 0) e2 => ANum 0  (* 5 *)
(*  | AMult  e1 (ANum 0) => ANum 0  (* 6 *) *)
(*  | AMult  (ANum 1) e2 => my_optimize e2  (* 7 *) *)
(*  | AMult  e1 (ANum 1) => my_optimize e1  (* 8 *) *)
  | AMult  e1 e2 => AMult  (my_optimize e1) (my_optimize e2)
  end.

Theorem my_optimize_sound : forall a,
  aeval (my_optimize a) = aeval a.
Proof.
	intros. induction a.
	- simpl. reflexivity.
	- destruct a1 eqn:Ea1.
		+ destruct n eqn:En.
			...
			* simpl. rewrite IHa2. reflexivity.
			(** this part is used to prove |APlus e1 (ANum 0) => my_optimize e1 **)
			(* * generalize dependent a1. intros.
				destruct a2 eqn:Ea2.
				** destruct n1 eqn:En1.
...
(* reduced by 29 *)
Theorem my_optimize_sound_auto : forall a,
  aeval (my_optimize a) = aeval a.
Proof.
	intros. induction a;
	try reflexivity.
	try (destruct a1; try (destruct n; simpl in *;
					[ apply IHa2 | rewrite IHa2; reflexivity]);
...
(*
Inductive beval_rules : bexp -> bool -> Prop :=
  | E_BTrue :
      beval_rules BTrue true
  | E_BFalse :
      beval_rules BFalse false
  | E_BEq a1 a2 :
      beval_rules (BEq a1 a2) ((aeval a1) =? (aeval a2))
  | E_BLe a1 a2 :
      beval_rules (BLe a1 a2) ((aeval a1) <=? (aeval a2))
  | E_BNot b b':
			beval_rules b b' -> beval_rules (BNot b) (negb b')
  | E_BAnd b1 b2 b1' b2':
      beval_rules b1 b1' -> beval_rules b2 b2' ->
			beval_rules (BAnd b1 b2) (b1' && b2').
*)
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BFalse :
      BFalse ==>b false
  | E_BEq : forall a1 a2 a1' a2',
      a1 ==> a1' -> a2 ==> a2' -> BEq a1 a2 ==>b  a1' =? a2'
  | E_BLe : forall a1 a2 a1' a2',
       a1 ==> a1' -> a2 ==> a2' -> BLe a1 a2 ==>b a1' <=? a2'
  | E_BNot : forall b b',
			b ==>b b' -> (BNot b) ==>b (negb b')
...
Proof.
	...
	- intros. induction H;
		...
		try (simpl in *; rewrite aeval_iff_aevalR in *;
					rewrite H, H0; reflexivity).
		+ simpl in *. rewrite IHbevalR in *. reflexivity.
		+ simpl in *. rewrite IHbevalR1, IHbevalR2. reflexivity.
	- generalize dependent bv.
		induction b;
		try (simpl; intros; subst; constructor);
		try (rewrite aeval_iff_aevalR; reflexivity).
		+ simpl. apply IHb. reflexivity.
		+ simpl. apply IHb1. reflexivity.
		+ simpl. apply IHb2. reflexivity.
Qed.

(* reduced by 14 *)
Lemma beval_iff_bevalR_auto : forall b bv,
  b ==>b bv <-> beval b = bv.
Proof.
	split.
	- intros. induction H; simpl; try (rewrite aeval_iff_aevalR in * );
		subst; reflexivity.
	- generalize dependent bv. induction b; simpl;
		try (intros; subst; constructor);
		try (rewrite aeval_iff_aevalR; reflexivity);
		try ( try apply IHb; try apply IHb1; try apply IHb2; reflexivity).
Qed.
(** [] *)
...
Inductive com : Type :=
  | CSkip
  | CAss (x : string) (a : aexp)  (* assign *)
  | CSeq (c1 c2 : com)  (* sequence *)
  | CIf (b : bexp) (c1 c2 : com)  (* if-else *)
  | CWhile (b : bexp) (c : com).  (* while loop *)
...
Proof.
	intros.
	inversion H. subst. inversion H2; subst. easy.
Qed.
*)
...
Lemma quiz3_answer : forall b c st st',
	st =[ if b then c else c end]=> st' ->
			st =[ c ]=> st'.
Proof.
	intros.
	inversion H. subst. easy. apply H6.
Qed.
*)
...
Proof.
	...
	induction WW; try discriminate Heqcc.
	- injection Heqcc; intros; subst.
...
Proof.
	...
	- apply E_WhileTrue with (X !-> 1; Y !-> 2; Y !-> 0; X !-> 2).
		...
		+ apply E_Seq with (Y !-> 2; Y !-> 0; X !-> 2).
			* apply E_Ass. reflexivity.
			* apply E_Ass. reflexivity.
		+ apply E_WhileTrue with (X !-> 0; Y !-> 3; 
					X !-> 1; Y !-> 2; Y !-> 0; X !-> 2).
			* reflexivity.
			* apply E_Seq with (Y !-> 3; 
					X !-> 1; Y !-> 2; Y !-> 0; X !-> 2).
				** apply E_Ass. reflexivity.
				** apply E_Ass. reflexivity.
...
Proof.
  ...
  (** Proceed by induction on the assumed derivation showing that
      ...
      [discriminate]). *)
	induction contra;
	try discriminate. inversion Heqloopdef; subst.
	- simpl in *. inversion H.
	- simpl in *. apply IHcontra2. apply Heqloopdef.
Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
	| no_whilesSKIP : no_whilesR CSkip
	| no_whilesASS : forall x v, no_whilesR <{ x := v }>
	| no_whilesSEQ : forall c1 c2, no_whilesR c1 -> no_whilesR c2
																	-> no_whilesR <{ c1 ; c2 }>
	| no_whilesIF : forall b ci ce, no_whilesR ci -> no_whilesR ce
																	-> no_whilesR <{ if b then ci else ce end }>
.
...
Proof.
	...
	- intros. induction c; try constructor;
		try (apply IHc1; inversion H; symmetry in H1; apply andb_true_eq in H1;
			destruct H1; rewrite <- H0, <- H1; simpl; reflexivity);
		try (apply IHc2; inversion H; symmetry in H1; apply andb_true_eq in H1;
			destruct H1; rewrite <- H0, <- H1; simpl; reflexivity).
		simpl in H. inversion H.
	- intros. induction H; try constructor;
		try (simpl in *; rewrite IHno_whilesR1, IHno_whilesR2; simpl; reflexivity).
Qed.

(* reduced by 15 *)
Theorem no_whiles_eqv_auto:
   forall c, no_whiles c = true <-> no_whilesR c.
Proof.
	split; intros.
	- induction c; try constructor; simpl in *;
		try (rewrite andb_true_iff in H; inversion H);
		try apply IHc1; try apply IHc2; try assumption; inversion H.
...
Proof.
	...
	- exists (x !-> (aeval st v) ; st). constructor. reflexivity.
	- destruct IHno_whilesR1 with st as [st''].
		destruct IHno_whilesR2 with st'' as [st'''].
		exists st'''. apply E_Seq with (st'').
		+ apply H1. + apply H2.
	- destruct IHno_whilesR1 with st as [sti].
		destruct IHno_whilesR2 with st as [ste].
		destruct (beval st b) eqn:E.
		+ exists sti. apply E_IfTrue. * apply E. * apply H1.
		+ exists ste. apply E_IfFalse. * apply E. * apply H2.
Qed.
...
Fixpoint s_execute (st : state) (stack : list nat)
	...
	match prog with
	| [] => stack
	| str :: t => match str with
			| SPush h => s_execute st (h :: stack) t
			| SLoad s => s_execute st ((st s) :: stack) t
			| SPlus => match stack with
									| x1 :: x2 :: t' => s_execute st ((x1 + x2) :: t') t
									| _ => s_execute st stack t
									end
			| SMinus => match stack with
									| x1 :: x2 :: t' => s_execute st ((x2 - x1) :: t') t
									| _ => s_execute st stack t
								end
...
Proof.
	...
	- simpl. destruct a.
		+ apply IHp1.
		+ apply IHp1.
		+ destruct stack. * apply IHp1. * destruct stack.
...
(* reduced by 12 *)
Theorem execute_app_auto : forall st p1 p2 stack,
    s_execute st stack (p1 ++ p2) = s_execute st (s_execute st stack p1) p2.
Proof.
	intros st p1. generalize dependent st.
	induction p1; intros.
	...
	- simpl. destruct a; auto;
		try (destruct stack; try (auto; destruct stack; auto)).
Qed.
(** [] *)
...
Proof.
	...
	try (rewrite execute_app; rewrite execute_app; rewrite IHe1;
		rewrite IHe2; reflexivity).
	- rewrite execute_app; rewrite execute_app. rewrite IHe1.
		rewrite IHe2. simpl.
		assert (a: aeval st e2 + aeval st e1 = aeval st e1 + aeval st e2).
		{ apply plus_comm. } rewrite a. reflexivity.
	- rewrite execute_app; rewrite execute_app. rewrite IHe1.
		rewrite IHe2. simpl.
		assert (a: aeval st e2 * aeval st e1 = aeval st e1 * aeval st e2).
		{ apply mult_comm. } rewrite a. reflexivity.
Qed.

(* reduced by 2 *)
Lemma s_compile_correct_aux_auto : forall st e stack,
  s_execute st stack (s_compile e) = aeval st e :: stack.
Proof.
	induction e; intros; try reflexivity; simpl;
	try (rewrite execute_app; rewrite execute_app; rewrite IHe1;
	...
	try (rewrite execute_app; rewrite execute_app; rewrite IHe1;
		rewrite IHe2; simpl).
	assert (a1: aeval st e2 + aeval st e1 = aeval st e1 + aeval st e2).
		{ apply plus_comm. } rewrite a1. reflexivity.
	assert (a2: aeval st e2 * aeval st e1 = aeval st e1 * aeval st e2).
		{ apply mult_comm. }
	try (rewrite execute_app; rewrite execute_app; rewrite IHe1;
		rewrite IHe2; simpl). rewrite a2. reflexivity.
Qed.
...
(* reduced by 1 *)
Theorem s_compile_correct_auto : forall (st : state) (e : aexp),
  s_execute st [] (s_compile e) = [ aeval st e ].
Proof.
	auto using s_compile_correct_aux.
Qed.
(** [] *)
