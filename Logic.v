...
Proof.
	...
	- intros. right. induction m as [| m' IHm].
			+ reflexivity.
			+ discriminate. 
Qed.
(** [] *)
...
(* Suppose we have a proposition P, and we need to prove (P->false)->false. If we have P=true, then true->false is false, false->anything, so false->false is true. If we have P=false, then false->anything, so false->false is true, then true->false is false. In all, in both conditions, we get P=~~P.*)
...
(* For a proposition P, we need to prove (P/\P->false)->false. When we have P=true, P->false is false, P/\P->false is false, then false->false is true. When we have P=false, P->false is true, P/\P->false is false, then false->false is also true. Therefore, no matter what P is, this is always true. *)
...
Proof.
	...
	split.
	- intros HPQR. split.
			+ destruct HPQR as [H1 | H2].	{ left. apply H1. }
					{ destruct H2 as [H3 H4]. right. apply H3. }
			+ destruct HPQR as [H1 | H2]. { left. apply H1. }
					{ destruct H2 as [H3 H4].  right. apply H4. }
	- intros HPQPR. destruct HPQPR as [HPQ HPR].
		destruct HPQ as [H1 | H2].
			+ left. apply H1.
			+ destruct HPR as [H3 | H4].
					{ left. apply H3. }
...
Proof.
	...
	- intros H. destruct H as [[x HEP] | [x HEQ]].
  ...
  intros A B f l y. split.
	- intros H. induction l.
		...
		+ destruct H. exists x. split.
			...
			* apply IHl in H. destruct H. exists x0.
				split. { destruct H as [H1 H2]. apply H1. }
				{ simpl. destruct H as [H1 H2]. right. apply H2. }
	- intros [x H]. destruct H as [H1 H2]. induction l.
		+ simpl in H2. destruct H2.
		+ destruct H2 as [H21 | H22].
			* simpl. left. rewrite H21. apply H1.
			* simpl. apply IHl in H22. right. apply H22.
Qed.
(** [] *)
...
Proof.
  intros A l. induction l as [|a' l' IH].
	- split.
		...
		+ intros H. simpl in H  . simpl. destruct H as [H | H].
			{ destruct H. } { apply H. }
	- split.
...
Proof.
	...
	- intros H. induction l.
		...
		+ intros. simpl in H0. simpl in H. destruct H0.
			* destruct H as [H1 H2]. rewrite <- H0. apply H1.
			* destruct H as [H1 H2]. apply IHl.
				{ apply H2. } { apply H0. }
Qed.
(** [] *)
...
Proof.
	intros Podd Peven n H0 H1.
	induction n.
	- unfold combine_odd_even in H0. rewrite H1 in H0. apply H0.
	- unfold combine_odd_even in H0. rewrite H1 in H0. apply H0.
Qed.
...
Theorem rev_app_outer : forall (X : Type) (l s t : list X),
	rev_append l (s ++ t) = (rev_append l s) ++ t.
Proof.
	intros X l.
	induction l as [| x' l'].
	- simpl. reflexivity.
	- intros s t. simpl. assert (a: x' :: s ++ t = (x' :: s) ++ t).
			{ simpl. reflexivity. }
		rewrite a. apply IHl' with (s := (x' :: s)).
Qed.
...
Proof.
	...
	- simpl. rewrite <- IHl. unfold tr_rev. simpl.
		assert (a: rev_append l [x] = rev_append l ([ ] ++ [x])).
			{ reflexivity. }
		rewrite a. apply rev_app_outer.
Qed.
(** [] *)
...
Proof.
  (* Hint: Use the [evenb_S] lemma from [Induction.v]. *)
	...
	- rewrite evenb_S. destruct IHn. destruct (evenb n).
		...
		+ simpl. exists (S x). rewrite H. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
	intros. split.
	- intros H. split.
		+ destruct b1. { reflexivity. } { discriminate. }
...
Proof.
	...
	- intros H. destruct b1.
		+ left. reflexivity.
		+ right. destruct b2. { reflexivity. } { discriminate. }
...
Theorem eqb_neq : forall x y : nat,
  x =? y = false <-> x <> y.
Proof.
	intros. rewrite <- eqb_eq. split.
	- intros H. unfold not. intros Heq.
		rewrite H in Heq. discriminate Heq.
	- intros H. unfold not in H. destruct (x =? y).
		+ contradiction.
		+ reflexivity.
Qed.
(** [] *)
...
Proof.
	...
	- intros l2. split.
		+ destruct l2.
			* simpl. intros Hcon. discriminate Hcon.
			* simpl. rewrite andb_true_iff.
				rewrite H. intros H3. destruct H3 as [H31 H32].
				apply IHl1 in H32. rewrite H31. rewrite H32. reflexivity.
		+ intros H3. destruct l2.
			* simpl. discriminate.
			* simpl. rewrite andb_true_iff. split.
				(* use inversion: https://pjreddie.com/coq-tactics/#inversion *)
				{ apply H. inversion H3. reflexivity. }
				{ apply IHl1. inversion H3. reflexivity. }
Qed.
...
Proof.
	intros H X P. unfold not. intros H1 x.
	destruct (H (P x)).
	- apply H0.
	- unfold not in H0. exfalso. apply H1. exists x. apply H0.
Qed.
(** [] *)
