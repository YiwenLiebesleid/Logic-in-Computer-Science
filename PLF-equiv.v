...
Proof.
	...
	- inversion H; subst.
		+ unfold bequiv in *. simpl in *. rewrite Hb in H5. discriminate.
		+ apply H6.
	- apply E_IfFalse; try assumption.
		unfold bequiv in *. simpl in *. apply Hb.
Qed.
(** [] *)
...
Proof.
	...
	- inversion H; subst.
		...
		+ apply E_IfTrue. * simpl. rewrite H5. reflexivity.
			* apply H6.
	- inversion H; subst.
		+ apply E_IfFalse. * simpl in *. apply negb_true_iff in H5. apply H5.
			* apply H6.
		+ apply E_IfTrue. * simpl in *. apply negb_false_iff in H5. apply H5.
			* apply H6.
Qed.
(** [] *)
...
Proof.
	...
	- exfalso. generalize dependent H. apply while_true_nonterm. auto.
	- exfalso. generalize dependent H. unfold bequiv in *.
...
Theorem assign_aequiv : forall (x : string) a,
  ...
  cequiv <{ skip }> <{ x := a }>.
Proof.
	intros. split; intros; unfold aequiv in *.
	inversion H0.
	- assert (Ha: st = t_update st x (aeval st a)).
		{ apply functional_extensionality. intros. rewrite <- H.
			rewrite t_update_same. reflexivity. }
		rewrite <- H3.
		assert (Ha2: st =[ x := a ]=> (x !-> aeval st a; st) ).
		{ apply E_Ass. reflexivity. }
		rewrite <- Ha in Ha2. apply Ha2.
	- inversion H0; subst.
		assert (Ha: st = t_update st x (aeval st a)).
		{ apply functional_extensionality. intros. rewrite <- H.
			rewrite t_update_same. reflexivity. }
		rewrite <- Ha. apply E_Skip.
Qed.
(** [] *)
...
Proof.
  intros. intros st st'. split.
	- intros H1. inversion H1; subst. apply E_Seq with st'0.
		unfold cequiv in H. rewrite H in H4. apply H4.
		unfold cequiv in H0. rewrite H0 in H7. apply H7.
	- intros H1. inversion H1; subst. apply E_Seq with st'0.
		unfold cequiv in *.
		+ apply H, H4. + apply H0, H7.
Qed.
(** [] *)
...
Proof.
	...
	- intros. inversion H2; subst.
		+ apply E_IfTrue. * unfold bequiv in H. rewrite <- H. apply H8.
			* unfold cequiv in H0. rewrite <- H0. apply H9.
		+ apply E_IfFalse. * unfold bequiv in H. rewrite <- H. apply H8.
			* unfold cequiv in H1. rewrite <- H1. apply H9.
	- intros. inversion H2; subst.
		+ apply E_IfTrue. * unfold bequiv in H. rewrite H. apply H8.
			* unfold cequiv in H0. rewrite H0. apply H9.
		+ apply E_IfFalse. * unfold bequiv in H. rewrite H. apply H8.
...
(* If we consider a hash function H, which doesn't always satisfy:
		H(x)=H(y) => H(x+1)=H(y+1).
	Then if we have H(x)=H(y), we know that x is equivalent to y, and this shows the equivalence. However, if we plus 1 to both x and y, then we no longer have H(x)=H(y), which means H doesn't satisfy congruence.
 *)
...
Proof.
  ...
  - (* BLe *)
    simpl. remember (fold_constants_aexp a1) as a1' eqn:Ha1'.
...
Proof.
  ...
  - (* while *)
    remember (fold_constants_bexp b) as b' eqn:Hb'.
		assert (bequiv b b').
		{ rewrite Hb'. apply fold_constants_bexp_sound. }
...
Proof.
	...
	- destruct a1. + destruct n; try (simpl; rewrite <- IHa2; reflexivity).
		+ simpl. rewrite <- IHa2. reflexivity.
		+ simpl in *. rewrite IHa1, IHa2. reflexivity.
		+ simpl in *. rewrite IHa1, IHa2. reflexivity.
		+ simpl in *. rewrite IHa1, IHa2. reflexivity.
	- destruct a1. + destruct n; try (simpl; rewrite <- IHa2; reflexivity). simpl. reflexivity.
...
Definition optimize_combination (c:com) : com :=
	optimize_0plus_com (fold_constants_com c).

Example optimize_combination_test :
	optimize_combination (
		 <{ X := ((ANum 2) + (ANum 3)) * ((ANum 0) + (AId Y)) }>
	) = <{ X := (ANum 5) * (AId Y) }>.
Proof. reflexivity. Qed.

Theorem optimize_combination_sound :
	ctrans_sound optimize_combination.
Proof.
	unfold ctrans_sound. intros. unfold optimize_combination.
...
Example subst_aexp_ex :
  ...
  = <{ Y + (42 + 53)}>.
Proof.  (* KK: For some reason this fails... Is it an associativity issue? *) simpl.
       Admitted.
       (* reflexivity.  Qed. *)
...
Proof.
	...
	try reflexivity; inversion H; subst;
	try (apply t_update_neq; apply H1);
	try (rewrite IHa1; try (rewrite IHa2; try reflexivity; try apply H3); try apply H2).
Qed.
...
Proof.
	...
	- rewrite eqb_string_true_iff in E. rewrite <- E. rewrite t_update_eq.
...
Proof.
	...
	- inversion H0; subst. apply E_Seq with st'0.
		+ apply H3. + inversion H3; subst. inversion H6; subst.
			apply E_Ass. symmetry. apply aeval_subst. apply H.
	- inversion H0; subst. apply E_Seq with st'0.
		+ apply H3. + inversion H6; subst. inversion H3; subst.
			apply E_Ass. apply aeval_subst. apply H.
Qed.
(* [] *)
