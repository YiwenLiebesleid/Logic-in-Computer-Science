...
Proof.
	intros X l1 l2.
	induction l1 as [| nl1' IHl1'].
	- simpl. reflexivity.
	- simpl. rewrite -> IHIHl1'. reflexivity.
Qed.
(** [] *)
...
Proof.
	...
	- simpl. rewrite -> IHIHl1'. rewrite <- app_assoc. reflexivity.
Qed.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
	filter evenb (filter (fun l1 => negb(l1 <=? 7)) l).
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
	((filter test l) , (filter (fun l1 => negb(test l1)) l)).
...
Lemma map_comm : forall (X Y : Type) (f : X -> Y) (l1 : list X) (l2 : X),
	map f (l1 ++ [l2]) = map f (l1) ++ [f l2].
Proof.
	intros X Y f l1 l2.
	induction l1 as [| nl1' IHl1'].
	- simpl. reflexivity.
...
Proof.
	...
	induction l as [| nl' IHl'].
	- simpl. reflexivity.
	- simpl. rewrite <- IHIHl'. rewrite -> map_comm. reflexivity.
Qed.
(** [] *)
...
Proof. unfold exp, plus. reflexivity. Qed.
